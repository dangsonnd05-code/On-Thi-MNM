...
(async () => {
      ...
      if (url.includes('/proceed') || url.includes('/check') || url.includes('/submit')) {
        ...
        if (data.questionId) {
          ...
          if (qObj) {
            ...
            // In Wayground/Quizizz, correctAnswers is an array of correct option IDs or an array of correct indices
            ...
            console.log(`Saved answer for: ${questionText.substring(0, 30)}...`);
          }
        }
      }
    } catch (e) {}
  ...
  for (let i = 0; i < 305; i++) {
    try {
      await page.waitForSelector('.option, [class*="option"], button', { timeout: 10000 });
      ...
      console.log('No options found, breaking loop.');
      break;
    }
  }
